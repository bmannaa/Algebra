...
ord' :: (Field k, Eq k, Show k) => R k -> Integer
ord' f = toInteger $ length m 
      where Mon m = lm f

-- | the number of indeterminates in the polynomial
upOrd' :: (Field k, Eq k, Show k) => UPoly (R k) x -> Integer
upOrd' f@(UP xs) | null xs = 1 
                 | otherwise = maximum (map ord' xs) + 1
-- | instance declaration of Ord R k
instance (Field k, Eq k, Show k) => Ord (R k) where
...
keyList (Mon xs)        | null ks = []
                        | n == 0       = error "last elem in monom is of 0 deg"
...
poly :: (Field k, Eq k, Show k) =>  R k -> UPoly (R k) x
poly f@(M bb) | f==zero = UP []
            ...
            where
                ...
                alterKey k = if k > lm f then error "weird key"
...
mpoly :: (Field k, Eq k, Show k) => Integer -> UPoly (R k) x -> R k
...
inverse i | (ord' i) == 0 = ST (\st -> [(st, if lc i == zero then Left () else Right $ toMPoly [(inv $ lc i,[])])])
          | otherwise     = do st <- get 
                               ...
                               let m = rank st
                               case (n > m) of
                                 True -> error "element belong to superfield"
                                            ...
                                            case g_deg > 0 of
                                              True  -> case g_deg == p_deg of
                                                         False -> do updateState n $ mpoly' g
                                                                     ...
                                                                     s2 <- get
                                                                     (e,_,y,_,_) <- iGCD g p' --y should always be of deg 0
...
revertState :: (Field k, Eq k, Show k) => Integer -> ST (S k) ()
revertState n = do st <- get
                   case (rank st < n) of
                    True  -> error "current state rank is less than required state"
                    False -> put $ take (fromInteger n) st
-- | appendToState
appendToState :: (Field k, Eq k, Show k) => R k -> ST (S k) ()
appendToState s = do st   <- get 
                     smod <- modPrevState s
                     put $ st ++ [smod]
-- | updateState 
updateState :: (Field k, Eq k, Show k) => Integer -> R k -> ST (S k) ()
...
modState r = do st <- get
                ...
                case n of 
                 ...
                 _ -> do let mpoly' = mpoly n
                         ...
                         let (UP ps) = poly r
                         ps' <- mapM modState ps
                         r' <- (UP ps') `modulo` (poly s)
                         return $ (mpoly' r')
                         
-- | simple division based on the inverse function
(/^) :: (Field k, Eq k, Show k) => R k -> R k -> ST (S k) (Either () (R k))
r1 /^ r2 = do _ <- get
              r2Inv <- inverse r2
              case r2Inv of 
                    Left () -> return $ Left () 
...
iGCD :: (Field k, Eq k, Show k) => UPoly (R k) x -> UPoly (R k) x -> ST (S k) (Quint (UPoly (R k) x))
...
iGCD' :: (Field k, Eq k, Show k) => UPoly (R k) x -> UPoly (R k) x -> Tup (UPoly (R k) x) -> Tup (UPoly (R k) x) -> ST (S k) (Quint (UPoly (R k) x))
...
iszero :: (Field k, Eq k, Show k) => UPoly (R k) x -> ST (S k) Bool
iszero g = do case g of 
                UP []     -> return $ True 
                UP (x:xs) -> do testInv <- inverse $ UPoly.lt g
                                case testInv of
                                  Left () -> iszero (UP $ init (x:xs))
                                  Right _ -> return $ False


-- | isSqFr test for square freeness 
isSqFr :: (Num k, Field k, Eq k, Show k) => UPoly (R k) x -> ST (S k) Bool
...
sqfr :: (Num k, Field k, Eq k, Show k) => UPoly (R k) x -> ST (S k) (UPoly (R k) x)
...
-- | root
-- function, a witnes that R k is algebraicly closed. given a polynomial returns a root of the polynomial
root :: (Num k, Field k, Eq k, Show k) => UPoly (R k) x -> ST (S k) (R k)
root p = do st <- get
            let m = rank st
            let n = upOrd' p
            case m < n - 1 of
              True  -> error "polynomial is over a super field"
